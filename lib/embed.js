...
const output = fs.createWriteStream('./embed.stdout.log');
const errorOutput = fs.createWriteStream('./embed.stderr.log');
const logger = new Console(output, errorOutput);

function embed(image, text, args) {
  settings.define(args.system);
  ...
  .then(([png, input, key]) => {
    ...
    const inputData = convertService.transformInput(input, logger);
    logger.log('inputData', inputData);
...
function harmUnused(background) {
  const maxHarm = settings.key.length - 1
