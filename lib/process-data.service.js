...
function hide(image, text, args) {
  ...
  .then(([png, key]) => {
    ...
    const inputData = utils.fromStr(text, args) + utils.padLeft(ETXCode, defaults.codeLength);
    const matrix = matrixService.create(key, args);
    const background = backgroundService.init(png.data, args, offset);
    console.log(background.all());
...
function parse(image, args) {
  ...
  .then(([png, key]) => {
    ...
    while (!parseCompleted && index < png.data.length) {
      ...
      if (res.slice(0-defaults.codeLength) === utils.padLeft(ETXCode, defaults.codeLength)) {
        parseCompleted = true;
        res = res.slice(0, 0-defaults.codeLength); //remove terminator char
      }
    }
