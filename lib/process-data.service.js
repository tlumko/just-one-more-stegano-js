...
function parse(image, args) {
  ...
  .then(([background, key]) => {
    ...
    output = utils.transformOutput(output);
...
function resolveBackground(image, args) {
  return utils
    .parseImage(image)
    .then(png => backgroundService.init(png.data))
    .then(background => [background, propsService.decode(background)])
    .then(([background, props]) => [background, utils.resolvePassword(props)])
