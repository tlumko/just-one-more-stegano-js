...
function transformInput(input) {
  ...
  input.data = convert(input.data).from(16).to(defaults.base);
  input.name = convert(input.data).from(16).to(defaults.base);
  return input.data + defaults.ETXChar + input.name + defaults.ETXChar;
}

function transformOutput(output, props) {
  output.data = convert(output.data).from(defaults.base).to(16);
  output.name = convert(output.name).from(defaults.base).to(16);
  ...
  if (props.unicode) {
    output.data = unicodeToHex(output.data);
    output.name = unicodeToHex(output.name);
  }
